...
const ipfs = {
  add: async (data, fileType = 'application/json') => {
    // Always use localStorage for testing to ensure consistency
    console.log('Using localStorage for IPFS simulation in testing mode');
    const mockCID = 'bafybeihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku' + Date.now();
    localStorage.setItem(mockCID, data);
    return { path: mockCID };
  }
};
