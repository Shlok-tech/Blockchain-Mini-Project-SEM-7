...
const Patient = ({mediChain, account, ethValue}) => {
  ...
  const handleShowRecordModal = async () => {
    // Always fetch the latest patient data to get updated record hash
    const latestPatient = await mediChain.methods.patientInfo(account).call();
    setPatient(latestPatient);

    var record = {}
    const mockCID = 'bafybeihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku';
    if (latestPatient.record.startsWith(mockCID)) {
      // For testing with localStorage mock CID, retrieve from localStorage
      const storedData = localStorage.getItem(latestPatient.record);
      if (storedData) {
      ...
      try {
        ...
        const url = `${gateway}${latestPatient.record}`;
        ...
        alert('Failed to load patient records. Please try again.');
      }
    }
  }
