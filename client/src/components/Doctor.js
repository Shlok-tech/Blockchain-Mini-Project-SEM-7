...
const Doctor = ({ipfs, mediChain, account}) => {
  ...
  const handleShowRecordModal = async (patient) => {
    var record = {}
    try {
      // First try localStorage (for testing mode)
      const storedData = localStorage.getItem(patient.record);
      if (storedData) {
        record = JSON.parse(storedData);
      } else {
        // Fallback to IPFS
        const gateway = process.env.REACT_APP_PINATA_GATEWAY || 'https://ipfs.io/ipfs/';
        const url = `${gateway}${patient.record}`;
  ...
  const submitDiagnosis = async (e) => {
    ...
    if(fileBuffer) {
      try {
        const result = await ipfs.add(fileBuffer, fileType);
        file = result.path;
      } catch (error) {
    ...
    try {
      const result = await ipfs.add(record);
